...
const TASK_DOMAIN_DEFINITIONS: TaskDomainDefinition[] = [
  ...
  {
    name: "setup",
    description: "Tasks for /api/setup",
    basePath: "/api/setup",
  },
  {
    name: "space",
    description: "Tasks for /api/spaces",
    basePath: "/api/spaces",
  },
  {
    name: "thread",
    description: "Tasks for /api/threads",
    basePath: "/api/threads",
    watchEnabled: true,
  },
  {
    name: "run",
    description: "Tasks for /api/runs",
    basePath: "/api/runs",
    watchEnabled: true,
    runFollow: true,
  },
  {
    name: "task",
    description: "Tasks for /api/agent-tasks",
    basePath: "/api/agent-tasks",
  },
  {
    name: "repo",
    description: "Tasks for /api/repos (includes pulls, actions)",
    basePath: "/api/repos",
    watchEnabled: true,
    actionsFollow: true,
  },
  ...
  { name: "git", description: "Tasks for /api/git", basePath: "/api/git" },
  {
    ...
    aliases: ["cap"],
    description:
      "Tasks for skills and tools (/api/spaces/*/skills, /api/spaces/*/tools, /api/skills)",
    basePath: "/api",
  },
  {
    name: "context",
    aliases: ["ctx"],
    description: "Tasks for memory and reminder endpoints under /api/*",
    basePath: "/api",
  },
  {
    name: "shortcut",
    description: "Tasks for /api/shortcuts",
    basePath: "/api/shortcuts",
  },
  {
    name: "notification",
    description: "Tasks for /api/notifications",
    basePath: "/api/notifications",
  },
  {
    name: "public-share",
    description: "Tasks for /api/public/thread-shares",
    basePath: "/api/public/thread-shares",
  },
  {
    name: "auth",
    description: "Tasks for /api/auth/* and /api/me/oauth/*",
    basePath: "/api",
  },
  {
    name: "discover",
    description: "Tasks for discovery/search endpoints under /api",
    basePath: "/api",
  },
];
...
function applySharedOptions(
  ...
  command
    ...
    .option(
      "-H, --header <key=value>",
      "Additional header (repeatable)",
      collectRepeatable,
      [],
    )
    .option("--space <id>", "Override X-Takos-Space-Id header")
  ...
  if (opts?.includeBody) {
    command
      ...
      .option(
        "--form-file <key=path>",
        "Multipart form file field (repeatable)",
        collectRepeatable,
        [],
      )
...
function registerCrudTaskCommands(command: Command, basePath: string): void {
  const crudDefinitions: Array<{
    ...
    {
      name: "list",
      description: "List resources (GET)",
      method: "GET",
      argSpec: "[target]",
    },
    {
      name: "view",
      description: "View resource details (GET)",
      method: "GET",
      argSpec: "[target]",
    },
    {
      name: "create",
      description: "Create resource (POST)",
      method: "POST",
      argSpec: "[target]",
    },
    {
      name: "replace",
      description: "Replace resource (PUT)",
      method: "PUT",
      argSpec: "<target>",
    },
    {
      name: "update",
      description: "Update resource (PATCH)",
      method: "PATCH",
      argSpec: "<target>",
    },
    {
      name: "remove",
      description: "Remove resource (DELETE)",
      method: "DELETE",
      argSpec: "<target>",
    },
    {
      name: "probe",
      description: "Probe resource availability (HEAD)",
      method: "HEAD",
      argSpec: "[target]",
    },
    {
      name: "describe",
      description: "Describe resource capabilities (OPTIONS)",
      method: "OPTIONS",
      argSpec: "[target]",
    },
  ];
  ...
  for (const def of crudDefinitions) {
    const subCommand = applySharedOptions(
      command.command(`${def.name} ${def.argSpec}`).description(
        def.description,
      ),
      { includeBody: true },
    );
    ...
    subCommand.action(
      async (target: string | undefined, options: ApiCommandOptions) => {
        await executeApiRequest(
          def.method,
          resolveTaskPath(basePath, target),
          options,
        );
      },
    );
...
function parseTransport(transportInput: string | undefined): "ws" | "sse" {
  ...
  if (transport !== "ws" && transport !== "sse") {
    console.log(
      red(`Unsupported transport: ${transportInput}. Use ws or sse.`),
    );
    cliExit(1);
  }
  return transport;
...
function registerWatchTaskCommand(command: Command, basePath: string): void {
  const watchCommand = applySharedOptions(
    ...
    .option(
      "--send <message>",
      "Send message after WS connection (repeatable)",
      collectRepeatable,
      [],
    );
...
function registerActionsFollowTask(command: Command): void {
  ...
  followCommand.action(
    async (repoId: string, runId: string, options: WatchTaskOptions) => {
      const transport = parseTransport(options.transport);
      if (transport !== "ws") {
        console.log(
          red("`takos repo follow` currently supports WebSocket only."),
        );
        cliExit(1);
      }

      await executeWebSocketStream(
        buildActionsWatchPath(repoId, runId),
        options,
      );
    },
  );
  ...
  for (const removed of REMOVED_HTTP_STYLE_SUBCOMMANDS) {
    ...
    command
      ...
      .action(() => {
        ...
        console.log(
          yellow(
            "Use task-style subcommands: list/view/create/replace/update/remove/probe/describe/watch/follow",
          ),
        );
        cliExit(1);
      });
