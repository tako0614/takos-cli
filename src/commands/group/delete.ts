...
export function registerGroupDeleteCommand(groupCmd: Command): void {
  groupCmd
    ...
    .action(
      async (
        name: string,
        options: { force?: boolean; offline?: boolean; space?: string },
      ) => {
        ...
        if (!options.offline) {
          ...
          try {
            ...
            if (!options.force) {
              ...
              console.log(
                dim(
                  "  Empty groups only. Delete fails if resources/services are still attached.",
                ),
              );
              console.log("");

              const confirmed = await confirmPrompt(
                bold(red("Delete this group?")),
              );
              if (!confirmed) {
        ...
        const totalCount = resourceCount + workerCount + containerCount +
          serviceCount;
