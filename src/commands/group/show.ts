...
import { getStateDir, readState } from "../../lib/state/state-file.ts";
import { cliExit } from "../../lib/command-exit.ts";
import { api } from "../../lib/api.ts";
import {
  requireApiGroupByName,
  resolveGroupSpaceId,
  toAccessOpts,
  validateGroupName,
} from "./helpers.ts";
...
export function registerGroupShowCommand(groupCmd: Command): void {
  groupCmd
    ...
    .action(
      async (
        name: string,
        options: { json?: boolean; offline?: boolean; space?: string },
      ) => {
        ...
        if (!options.offline) {
          try {
            ...
            const res = await api<GroupDetailResponse>(
              `/api/spaces/${spaceId}/groups/${group.id}`,
            );
            if (!res.ok) {
            ...
            if (resources.length > 0) {
              ...
              for (const resource of resources) {
                ...
                const typeLabel = resource.manifestType
                  ? `[${String(resource.manifestType)}]`
                  : "";
                const idLabel = resource.resourceId
                  ? dim(` (${String(resource.resourceId)})`)
                  : "";
                console.log(
                  `  resources.${resourceName} ${typeLabel}${idLabel}`,
                );
              }
              console.log("");
            }
            ...
            if (workloads.length > 0) {
              ...
              for (const workload of workloads) {
                ...
                const hostLabel = workload.hostname
                  ? dim(` -> ${String(workload.hostname)}`)
                  : "";
                console.log(
                  `  ${sourceKind}s.${workloadName} [${sourceKind}]${hostLabel}`,
                );
              }
              console.log("");
            }
            ...
            if (routes.length > 0) {
              ...
              for (const route of routes) {
                ...
                const urlLabel = route.url
                  ? dim(` url=${String(route.url)}`)
                  : "";
            ...
            const totalCount = resources.length + workloads.length +
              routes.length;
            if (totalCount === 0) {
        ...
        if (workerKeys.length > 0) {
          ...
          for (const workerName of workerKeys) {
            ...
            const scriptLabel = worker.scriptName
              ? dim(` -> ${worker.scriptName}`)
              : "";
        ...
        if (routeKeys.length > 0) {
          ...
          for (const routeName of routeKeys) {
            ...
            const domainLabel = route.domain
              ? dim(` domain=${route.domain}`)
              : "";
            const urlLabel = route.url ? dim(` url=${route.url}`) : "";
            console.log(
              `  routes.${routeName} -> ${route.target}${domainLabel}${urlLabel}`,
            );
          }
          console.log("");
        }

        const totalCount = resourceKeys.length + workerKeys.length +
          containerKeys.length + serviceKeys.length + routeKeys.length;
        if (totalCount === 0) {
          console.log(dim("Group is empty."));
        }
