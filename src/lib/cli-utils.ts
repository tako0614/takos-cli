...
export function resolveAccountId(override?: string): string {
  ...
  if (!accountId.trim()) {
    ...
    console.log(
      dim("Set CLOUDFLARE_ACCOUNT_ID or configure it in local config."),
    );
    cliExit(1);
  }
...
export function resolveApiToken(override?: string): string {
  ...
  if (!apiToken.trim()) {
    ...
    console.log(
      dim("Set CLOUDFLARE_API_TOKEN or configure it in local config."),
    );
    cliExit(1);
  }
...
/**
 * Resolve the target space ID from an explicit override,
...
export function resolveSpaceId(spaceOverride?: string): string {
  ...
  if (!spaceId) {
    console.log(
      red(
        "Space ID is required. Pass --space or configure a default space.",
      ),
    );
    cliExit(1);
  }
