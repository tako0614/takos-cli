 ...
 * authenticated (takos token/session).
 */
 ...
 */
export async function readGroupStateFromApi(
  group: string,
 ...
 */
export async function writeGroupStateToApi(
  group: string,
  state: TakosState,
): Promise<void> {
      ...
      case "worker":
        state.workers[entity.name] = {
          ...
          ...(Array.isArray(config.containers)
            ? { containers: config.containers as string[] }
            : {}),
        };
        break;
      case "container":
...
function stateToApiPayload(state: TakosState): {
  ...
  groupName: string;
  entities: Array<
    { category: string; name: string; config: Record<string, unknown> }
  >;
} {
