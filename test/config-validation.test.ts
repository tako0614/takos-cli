...
Deno.test("validateApiUrl - accepts HTTPS on api.takos.jp", () => {
  const result = validateApiUrl("https://api.takos.jp");
  assertEquals(result, { valid: true });
});
Deno.test("validateApiUrl - accepts HTTPS on an example domain", () => {
...
Deno.test("validateApiUrl - accepts HTTPS on a custom domain", () => {
...
Deno.test("validateApiUrl - rejects invalid schemes on non-localhost", () => {
  const result = validateApiUrl("ftp://example.com");
  assertEquals(result.valid, false);
  assertStringIncludes(result.error ?? "", "HTTP or HTTPS");
});
...
Deno.test("validateApiUrl - rejects FTP on localhost", () => {
  const result = validateApiUrl("ftp://localhost:8787");
  assertEquals(result.valid, false);
  assertStringIncludes(result.error ?? "", "HTTP or HTTPS");
});
...
Deno.test("isValidId - accepts single character ID by default", () => {
...
Deno.test("isValidId - accepts ID with hyphen", () => {
  assertEquals(isValidId("my-space-123"), true);
});
...
Deno.test("isValidId - rejects short session IDs when minLength is 8", () => {
  assertEquals(isValidId("short", 8), false);
  assertEquals(isValidId("session1", 8), true);
});
